...
function custom_scripts()
{
    ...
    wp_localize_script('carousel-js', 'carouselData', array(
        'immage_id' => $image_id
    ));
}
...
// Add rewrite rules to handle custom URLs
...
{
    custom_rewrite_rules(); // Add the rewrite rules
...
function enqueue_hotspot_plugin_scripts()
{
    ...
    wp_enqueue_script(
        'hotspot-script',
        plugins_url('hotspot-plugin/js/hotspot-script.js'),
        array('jquery'),
        '1.0',
        true
    );

    // Enqueue your plugin style
    wp_enqueue_style(
        'hotspot-style',
        plugins_url('hotspot-plugin/css/style.css'),
        array(),
        '1.0',
        'all'
    );
